...
@interface DDQExpiredCouponController ()

@property (nonatomic, strong) NSArray *expired_dataSource;

@end

@implementation DDQExpiredCouponController
...
- (void)viewDidLoad {
    ...
    //NetRequest
    DDQWeakObject(self);
    [self handler_requestCouponDataWithType:@"3" completed:^(NSArray<DDQCouponModel *> * _Nonnull dataSource) {
        
        weakObjc.expired_dataSource = dataSource;
...
- (void)base_tableViewConfig {
    ...
    [self.base_tableView tableView_setCellConfig:^__kindof UITableViewCell * _Nonnull(NSIndexPath * _Nonnull indexPath, DDQFoundationTableView * _Nonnull tableView) {
        ...
        [cell cell_updateDataWithModel:weakObjc.expired_dataSource[indexPath.section]];
        tableView.tableView_layout.layout_rowHeight = [DDQCouponCell cell_getCellHeightWithModel:weakObjc.expired_dataSource[indexPath.section]];
        return cell;
...
- (DDQFoundationTableViewLayout *)base_tableViewLayout {
    ...
    layout.layout_sectionCount = self.expired_dataSource.count;
