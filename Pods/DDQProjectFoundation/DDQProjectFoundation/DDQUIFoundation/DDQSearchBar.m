...
- (void)textFieldDidBeginEditing:(UITextField *)textField {
    ...
    if (self.searchDelegate && [self.searchDelegate respondsToSelector:@selector(searchBar_beginEditingWithSearchBar:)]) {
        [self.searchDelegate searchBar_beginEditingWithSearchBar:self];
    }
}

- (void)textFieldDidEndEditing:(UITextField *)textField {
    
    if (self.searchDelegate && [self.searchDelegate respondsToSelector:@selector(searchBar_endEditingWithSearchBar:)]) {
        [self.searchDelegate searchBar_endEditingWithSearchBar:self];
    }
}

- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string {
    
    if (self.searchDelegate && [self.searchDelegate respondsToSelector:@selector(searchBar_text:relpaceText:range:searchBar:)]) {
        [self.searchDelegate searchBar_text:textField.text relpaceText:string range:range searchBar:self];
    }
    return YES;
}
