...
- (id) foundation_handleResponseObject:(id)object {
    ...
    if ([object isKindOfClass:[NSData class]]) {
        ...
        if (!json) {
            ...
            //为什么要这么写呢？
            //因为会出现返回值前面有N多\0的情况，我需要把\0都去掉
            NSString *responseJosnString = [[NSString alloc] initWithData:object encoding:NSUTF8StringEncoding];
            responseJosnString = [responseJosnString stringByReplacingOccurrencesOfString:@"\0" withString:@""];
            NSData *responseData = [responseJosnString dataUsingEncoding:NSUTF8StringEncoding];
